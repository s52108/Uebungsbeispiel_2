    ...
    public ArrayList<User> findAllFriends(User user, int level, int maxDepth)
    {
        ...
        for(User u : user.getFriends())
        {
            if(!friends.contains(u))
                friends.add(u);
            addFriends(friends, findAllFriends(u, level, maxDepth));
        }

        return friends;
    }

    private ArrayList<User> addFriends(ArrayList<User> alreadyAddedFriends, ArrayList<User> newFriends)
    {
        for(User u : newFriends)
        {
            if(!alreadyAddedFriends.contains(u))
                alreadyAddedFriends.add(u);
        }

        return alreadyAddedFriends;
    }
